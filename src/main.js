 ...
 */
const createStatistic = () => `<p>130 291 movies inside</p>`;
...
const init = () => {
  ...
  renderElements(Count.FILMS, filmsContainer, createFilmCard());

  const filmsList = Nodes.MAIN.querySelector(`.films .films-list`);
  render(filmsList, createShowMore());
  render(filmsList, createMostCommentedFilms(), Position.AFTER_END);
  render(filmsList, createTopFilms(), Position.AFTER_END);

  const filmListsExtra = document.querySelectorAll(`.films-list--extra .films-list__container`);
  for (const itemExtra of filmListsExtra) {
    renderElements(Count.FILMS_EXTRA, itemExtra, createFilmCard());
  }
